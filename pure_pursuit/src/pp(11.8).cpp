...
private:
  ...
  void detectCornerHeavy(const sensor_msgs::msg::LaserScan& s, double steer){
    ...
    if (debug_markers_) {
      ...
      // ---------- ★ ADDED(연결 시각화): 인라이어 점 + 연결 선(안/밖) ----------
      // in/out 인라이어 포인트 수집
      std::vector<Pt> in_inliers;  in_inliers.reserve(in_idx.size());
      std::vector<Pt> out_inliers; out_inliers.reserve(out_idx.size());
      for (int ii: in_idx)  in_inliers.push_back(in_pts[(size_t)ii]);
      for (int oo: out_idx) out_inliers.push_back(out_pts[(size_t)oo]);

      // (d) 인라이어 점(안쪽=청록, 바깥=자홍)
      {
        visualization_msgs::msg::Marker p_in, p_out;
        p_in.header.stamp = p_out.header.stamp = stamp;
        p_in.header.frame_id = p_out.header.frame_id = debug_frame_;
        p_in.ns = p_out.ns = "pp_corner";
        p_in.id = 4; p_out.id = 5;
        p_in.type = p_out.type = visualization_msgs::msg::Marker::POINTS;
        p_in.action = p_out.action = visualization_msgs::msg::Marker::ADD;
        p_in.scale.x = p_in.scale.y = 0.03; p_in.scale.z = 0.03;
        p_out.scale.x = p_out.scale.y = 0.03; p_out.scale.z = 0.03;
        // 안쪽(청록)
        p_in.color.a = 1.0; p_in.color.r = 0.0; p_in.color.g = 1.0; p_in.color.b = 1.0;
        // 바깥(자홍)
        p_out.color.a = 1.0; p_out.color.r = 1.0; p_out.color.g = 0.0; p_out.color.b = 1.0;

        geometry_msgs::msg::Point gp;
        for (auto& q: in_inliers){ gp.x=q.x; gp.y=q.y; gp.z=0.02; p_in.points.push_back(gp); }
        for (auto& q: out_inliers){ gp.x=q.x; gp.y=q.y; gp.z=0.02; p_out.points.push_back(gp); }

        arr.markers.push_back(p_in);
        arr.markers.push_back(p_out);
      }

      // (e) 인라이어 연결선 LINE_STRIP(안/밖)
      auto make_sorted_strip = [&](const std::vector<Pt>& src,
                                   int id, float r,float g,float b){
        visualization_msgs::msg::Marker ls;
        ls.header.stamp = stamp; ls.header.frame_id = debug_frame_;
        ls.ns = "pp_corner"; ls.id = id;
        ...
        // 각도 기준으로 정렬(전방에서 부드럽게 연결)
        std::vector<std::pair<double,Pt>> tmp; tmp.reserve(src.size());
        for (auto& q: src) tmp.push_back({ std::atan2(q.y, q.x), q });
        std::sort(tmp.begin(), tmp.end(),
                  [](auto& A, auto& B){ return A.first < B.first; });

        geometry_msgs::msg::Point gp;
        for (auto& pr: tmp){ gp.x=pr.second.x; gp.y=pr.second.y; gp.z=0.01; ls.points.push_back(gp); }
        return ls;
      };

      if (!in_inliers.empty()){
        arr.markers.push_back(make_sorted_strip(in_inliers, 6, /*r*/0.0f, /*g*/1.0f, /*b*/1.0f)); // 청록
      }
      if (!out_inliers.empty()){
        arr.markers.push_back(make_sorted_strip(out_inliers, 7, /*r*/1.0f, /*g*/0.0f, /*b*/1.0f)); // 자홍
      }

      // (f) RANSAC 직선 자체를 선분으로 시각화(전방 x∈[0.05, intersect_max_dist_])
      auto line_to_strip = [&](const Line& L, int id, float r,float g,float b){
        visualization_msgs::msg::Marker ls;
        ls.header.stamp = stamp; ls.header.frame_id = debug_frame_;
        ls.ns = "pp_corner"; ls.id = id;
        ls.type = visualization_msgs::msg::Marker::LINE_STRIP;
        ...
        if (std::abs(L.b) > 1e-6) {
          for (double x=0.05; x<=xmax; x+=0.05){
            ...
            if (std::abs(y) > 2.0*ttc_corridor_half_) continue;
          ...
          if (x >= 0.05 && x <= xmax){
            for (double y=-2.0*ttc_corridor_half_; y<=2.0*ttc_corridor_half_; y+=0.05){
              gp.x=x; gp.y=y; gp.z=0.01; ls.points.push_back(gp);
            }
          }
        }
      ...
      arr.markers.push_back(line_to_strip(Lin, 8, /*r*/0.0f, /*g*/0.6f, /*b*/0.0f));   // 안쪽 직선(초록톤)
      arr.markers.push_back(line_to_strip(Lout, 9, /*r*/0.6f, /*g*/0.0f, /*b*/0.6f));  // 바깥 직선(보라톤)
